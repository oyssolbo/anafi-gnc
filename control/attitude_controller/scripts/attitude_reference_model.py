...
class GenericAttitudeReferenceGenerator():
    ...
    def __clamp(self, value: float, limits: tuple):
        if value < limits[0]:
            return limits[0]
        elif value > limits[1]:
            return limits[1]
...
class PIDReferenceGenerator(GenericAttitudeReferenceGenerator):
    ...
    def get_attitude_reference(
        ...
        pitch_reference = self.__clamp(pitch_reference, self._pitch_limits)
        roll_reference = self.__clamp(roll_reference, self._roll_limits)
...
class LinearDragModelReferenceGenerator(GenericAttitudeReferenceGenerator):
    ...
    def get_attitude_reference(self, v_ref: np.ndarray, v: np.ndarray, ts: float, debug=False):
        ...
        pitch_ref = self.__clamp(pitch_ref, self._pitch_limits)
        roll_ref = self.__clamp(roll_ref, self._roll_limits)
