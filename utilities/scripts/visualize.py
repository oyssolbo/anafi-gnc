...
def main():
    ...
    title = "Polled velocities versus optical flow"
    synced_vel_data = plotter.sync_multiple_data_series_based_on_timestamps([optical_flow, polled_vel])#, qualisys_drone_vel])
    plotter.plot_multiple_data_series(
        data=synced_vel_data, numplots=3, suptitle=title,
        legends=["Optical flow", "Polled velocity"], xlabels=["t [sec]", "t [sec]", "t [sec]"], ylabels=["vx[m]", "vy[m]", "vz[m]"],
